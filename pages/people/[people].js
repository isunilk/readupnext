...
const People = ({ data, errorCode }) => {
    ...
    const [details, setDetails] = useState(data ? { main_p: data[0].recommender, other_p: data[1] } : null);
    ...
    return (
        <>
            <Seo
                ...
                metaTitle={`${details ? details.main_p.booksCount : null} Books recommended by ${details ? details.main_p.name : null} (Complete List)`}
                canonical={pathName}
            ...
            <div className="container team-section-two d-flex flex-column align-items-center">
                <div className='d-flex align-items-center col-12 pt-30 justify-content-between' style={{ borderBottom: "3px solid #c475e3" }}>
                    <h2 className="fs-20 fw-500 tx-new w-75">{details ? details.main_p.booksCount : null} Books recommended by {details ? details.main_p.name : null}</h2>
                    <div className='me-2 col-3 text-end' style={{ fontSize: "25px" }}>
                        <i onClick={() => { setLayout(false) }} title='list layout' className="fa fa-list-ul me-2 cursor-pointer" aria-hidden="true"></i>
                        <i onClick={() => { setLayout(true) }} title="grid layout" className="bi bi-grid-fill cursor-pointer"></i>
                    </div>
                </div>
                <div className="pe-3 ps-3 ps-xxl-5 pe-xxl-5 col-lg-11" style={{ overflow: "hidden", display: changelay ? "block" : "none" }}>
                    <div className="row justify-content-center gap-2">
                        <PeopleBooks data={recomData} />
                    </div>
                </div>
                <div className='w-100' style={{ display: changelay ? "none" : "block" }}>
                    <PeopleList_layout data={recomData} person={details ? details.main_p : null} />
                </div>
            </div >
            {details ?
            ...
            <div className="container peopleCard wrapper border-bottom pb-120 lg-pb-80 position-relative mt-100">
                <h2 className='fs-20 text-center mb-50 text-decoration-underline'>Other people you might be interested in</h2>
                <div className="row justify-content-center gap-sm-3 gap-2">
                    ...
                    <div className="col-lg-2 col-md-3 col-sm-5 col-6 d-flex">
                        ...
                        >
                            <h3>750+</h3>
                            <p className="text-dark fw-500 text-center mx-1">Explore More People</p>
            ...
            <div className='position-fixed top-0 start-0 justify-content-center align-items-center w-100 h-100' style={{ display: recomData ? "none" : "flex", background: "#ffffffeb", zIndex: "20" }}>
                <SearchBar1 />
            </div>
            <DefaultFooter />
        </>
...
export async function getServerSideProps(Context) {
    ...
    const res = await fetch(`${process.env.NEXT_API_URL}/api/recommnended-books?name=${people}`)
