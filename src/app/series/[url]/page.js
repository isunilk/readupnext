...
import { listInternal, seriesInternal } from "@/app/api/fetch-data/[action]/action"
import { Share } from "@/app/author/[name]/Share"

const page = async ({ params }) => {
    await dbConnect();
    let data = await seriesInternal(params.url)
        ...
        <>
            ...
            <section className="container my-5">
                <div className={"d-flex justify-content-center align-items-center positon-relative " + style.list_banner}>
                    {
                        data.seriesDetails.imgArr.map((item, index) => (
                ...
                <div className={style.list_details}>
                    <h1 className="text-center">{data.seriesDetails.count} {data.seriesDetails.series_name} .</h1>
                    <p className="text-center">Lorem ipsum dolor sit, amet consectetur adipisicing elit. Ipsum non sit dolore aliquam voluptas officia odit possimus consequatur fugit quia similique sed, officiis ad iste omnis, porro deleniti, culpa perferendis?</p>
            ...
            <section className="container my-5">
                <h2 className={"text-center fw-semibold " + style.subtitle} >{data.seriesDetails.count} {data.seriesDetails.series_name}</h2>
            ...
            <section className="container my-5">
                <h2 className={"text-center fw-semibold " + style.subtitle} >Similar Series</h2>
                <div className="row">
                    {
                        data.similarSeries.map((item, index) => (
                            <Link href={"/list/" + item.slug} key={index} className="text-decoration-none text-dark col-4 py-3">
                                <div className={"p-2 rounded overflow-hidden " + style.listCard}>
                                    ...
                                    <div className="position-relative">
                                        <h3 className="text-center mt-3 fs-4 fw-semibold">{item.series_name}</h3>
