...
const DynamicBlogDetails = () => {
  ...
  const getBlog = async (title)=>{
    try{
      let res = await fetch(`/api/blog?title=${title}`)
      res = await res.json();
      setBlogItem(res[0]);
    }catch(err){
      console.log(err)
  ...
  return (
    <>
      <Seo pageTitle={blog?blog.title:null} />
      {/* <!-- 
      =============================================
      Theme Default Menu
      ============================================== 	
      ...
      {/* 
			...
			============================================== 
			*/}
      <div className="fancy-feature-fiftyOne position-relative mt-150 lg-mt-200">
        <div className="container">
          <div className="row">
            <div className="col-xxl-8 col-lg-9 wow fadeInLeft">
              ...
              <h2 className="blog-heading-one tx-dark">{blog?blog.title:null}</h2>
            </div>
          </div>
        </div>
        {/* /.container */}
