...
const List = ({ data, errorCode }) => {
    ...
    "@graph": [
      {
        ...
        itemListElement: [
          ...
          },
          {
            ...
            position: 3,
            item: {
              "@id": `https://www.readupnext.com/list/${similarCat.searchCategory[0].Best_Book_List.toLowerCase().replace(
                / /g,
                "-"
              )}`,
              name: "Best Books List Across 1300 Categories",
      ...
      {
        "@type": "CollectionPage",
        name: `${listBooks ? listBooks.length : 0} Best ${
          listBooks[0] ? listBooks[0].category_name : "category"
        } Books Of All Time (Updated 2024)`,
  ...
  return (
    <>
      ...
      <Seo
        pageTitle={`${listBooks ? listBooks.length : 0} Best ${
          listBooks[0] ? listBooks[0].category_name : "category"
        } Books Of All Time (Updated 2024)`}
        descr={similarCat ? similarCat.searchCategory[0].metaDescription : ""}
        metaTitle={`${listBooks ? listBooks.length : 0} Best ${
          listBooks[0] ? listBooks[0].category_name : "category"
        } Books Of All Time (Updated 2024)`}
        canonical={pathName}
        ogImage={
          similarCat.searchCategory[0].imgArr[0]
            ? similarCat.searchCategory[0].imgArr[0]
      ...
      <div className="hero-banner-twelve pt-120 sm-pt-120 pb-20 sm-pb-20 lg-pb-80 md-pt-200">
        <div className="container">
          <div className="row align-items-start justify-content-center justify-content-md-start">
            <div className="col-lg-4 col-sm-6 col-10 mb-4" data-aos="fade-up">
            ...
            <div className="col-sm-6 col-10 books-hero-secondDiv">
              <div className="text-wrapper" data-aos="fade-right">
                <h1 className="fs-2 text-center text-sm-start fw-500 tx-new">
                  {listBooks ? listBooks.length : 0} Best{" "}
                  {listBooks[0] ? listBooks[0].category_name : "category"} Books
                  Of All Time (Updated 2024).
                </h1>
                <p className="text-center text-md-start">
                  Check out the Best{" "}
                  {listBooks[0] ? listBooks[0].category_name : "category"} Books
                  - Picked based on recommendations from experts and fan around
                  the world.
                </p>
              </div>
            </div>
      ...
      <div className="container team-section-two mt-20 d-flex flex-column align-items-center">
        <div
          className="d-flex align-items-center col-12 justify-content-between mb-3"
          style={{ borderBottom: "3px solid #c475e3" }}
        >
          <h2 className="fs-20 tx-new mb-0 fw-500 w-75">
            {listBooks[0] ? listBooks.length : 0} Best Books On{" "}
            {listBooks[0] ? listBooks[0].category_name : "category"}
          </h2>
        ...
        <div className="mt-70">
          ...
          <div className="row justify-content-around mt-20">
            <ListMap
              data={similarCat ? similarCat.similarCategory : null}
              similarList={true}
            />
          </div>
          <div className="d-flex justify-content-center mb-20">
            <Link className="fw-500 viewAllBtn" href="/list">
      ...
      <div
        ...
        style={{
          display: listBooks.length > 0 ? "none" : "flex",
          background: "#ffffffeb",
          zIndex: "20",
        }}
      >
...
export async function getStaticProps({ params }) {
  ...
  const res = await fetch(
    `${process.env.NEXT_API_URL}/api/list-category-books?category=${list}`
  );
  const errorCode = res.ok ? false : res.status;
  const data = await res.json();
