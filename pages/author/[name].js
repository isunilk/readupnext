...
const SeriesDetail = ({ data, errorCode }) => {
    ...
    return (
        <>
            <Seo
                pageTitle={`All ${authorDetail ? authorDetail.name : "series"} Books in Order (Complete List)`}
                descr={`Find all ${seriesBooks ? seriesBooks.length : null} books written by ${authorDetail ? authorDetail.name : "series"}, all arranged in the order they were published. Discover the full collection in sequence`}
                metaTitle={`All ${authorDetail ? authorDetail.name : "series"} Books in Order (Complete List)`}
                canonical={pathName}
                            ...
                            <div className="image-holder zn2 d-inline-block position-relative" style={{ maxHeight: "542px", width: "fit-content", overflow: "hidden" }}>
                                {authorDetail ?
                                    <Image
                                        width={200}
                                        height={200}
                                        src={authorDetail.imageURL ? authorDetail.imageURL : "/images/assets/blank_people.jpg"}
                        ...
                        <div className="col-md-8 books-hero-secondDiv d-flex flex-column align-items-center align-items-md-start mt-25 mt-md-0">
                            <div className="text-wrapper" data-aos="fade-right">
                                <h1 className="fs-2 fw-500 tx-new text-center text-md-start">
                                    All {authorDetail ? authorDetail.name : null} Books In Order (Complete List)
                                </h1>
                                ...
                                <p className='text-center font-14 text-md-start mt-3 mb-0'>
                                    Explore all {seriesBooks.length} books written by {authorDetail.name}, all arranged in the order they were published.
                                </p>
            ...
            <div className="container team-section-two mt-20 d-flex flex-column align-items-center">
                <div className='d-flex align-items-center col-12 mb-3 justify-content-between' style={{ borderBottom: "3px solid #c475e3" }}>
                    <h2 className="fs-20 mb-0 tx-new fw-500 w-75" >All {authorDetail.name} Books in Order with their release dates.</h2>
            ...
            <div className="container peopleCard wrapper border-bottom pb-120 lg-pb-80 position-relative mt-100">
                <h2 className='fs-20 text-center mb-50 text-decoration-underline'>Other author you might be interested in</h2>
                <div className="row justify-content-center gap-sm-3 gap-2">
                    ...
                    <div className="col-lg-2 col-md-3 col-sm-5 col-6 d-flex">
                        <Link
                            style={{ background: "var(--prime-eleven)", borderRadius: "15px", minHeight: "13rem" }}
                            href="/author"
                            className="team-block-two d-flex flex-column justify-content-center align-items-center mb-40"
                        >
                            {/* <h3>750+</h3> */}
                            <p className="text-dark fw-500 text-center mx-1">Explore More Authors</p>
                            <img src="/images/icon/icon_134.svg" style={{ maxHeight: "20px" }} alt="" className="mx-auto" />
                        </Link>
                        {/* /.card-style-eighteen */}
                    </div>
                </div>
...
export async function getServerSideProps(Context) {
    ...
    const resData = await fetch(`${process.env.NEXT_API_URL}/api/series-author-books?name=${name.replace("-books-in-order", "")}`)
