...
"use client"
import style from "@/app/people/person.module.css"
import { usePathname } from "next/navigation"
import { useRouter } from "next/router"
import { useEffect, useState } from "react"
import { EmailIcon, EmailShareButton, FacebookIcon, FacebookShareButton, PinterestIcon, PinterestShareButton, RedditIcon, RedditShareButton, TelegramIcon, TelegramShareButton, TwitterIcon, TwitterShareButton, WeiboShareButton, WhatsappIcon, WhatsappShareButton } from "react-share"

export const Share = () => {
    const [openShare, setShare] = useState(false)
    const [copytext, setCopy ] = useState("Copy Link")
    const [url, setUrl] = useState('#')
    
    const handleOutter = (e)=>{
        const target = [...e.target.classList].includes("outterdiv");
        if(!target) return;
        setShare(false)
    }
    const copy_url = ()=>{
    ...
    return (
        <>
            ...
            <button className="border-0" onClick={()=>{setShare(true)}}>
            ...
            <div style={{display:openShare?"flex":"none"}} className={style.share + " justify-content-center align-items-center position-fixed top-0 start-0 w-100 h-100 outterdiv"} onClick={handleOutter}>
                <div>
                    <h3>Share in your Goup & Friends</h3>
                    ...
                    <button onClick={copy_url}>{copytext} <i className="bi bi-link-45deg"></i></button>
                </div>
            </div>
        </>
    )
