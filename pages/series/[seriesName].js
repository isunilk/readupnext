...
const SeriesDetail = ({ data, errorCode }) => {
    ...
    return (
        <>
            <Seo
                pageTitle={`${seriesDetail ? seriesDetail.series_name : "series"} Books in Order (by Author ${seriesBooks[0].authorName})`}
                descr={`Discover the ${seriesDetail ? seriesDetail.series_name : "series"} book series in their intended order. Experience the series seamlessly, ensuring you don't miss a single magical moment.`}
                metaTitle={`${seriesDetail ? seriesDetail.series_name : "series"} Books in Order (by Author ${seriesBooks[0].authorName})`}
                canonical={pathName}
                ogImage={seriesDetail.imgArr[0]?seriesDetail.imgArr[0]:"https://www.readupnext.com/opengraph_image.png"}
            />
            <Header1 />
            <Head>
                <script
                        ...
                        <div className="col-sm-6 col-lg-7 col-10 books-hero-secondDiv">
                            <div className="text-wrapper" data-aos="fade-right">
                                <h1 className="fs-2 text-center text-sm-start fw-500 tx-new">
                                    {seriesDetail.series_name} Books in Order (by Author {seriesBooks[0].authorName})
                                ...
                                <p className="d-flex justify-content-center justify-content-sm-start tx-dark mb-30 pt-lg-3 lg-mb-20">
                                    <span className="fw-bold">Author : </span> &nbsp; {seriesBooks[0].authorName}
                                </p>
                                <p className="text-center text-md-start">Find all “{seriesDetail.series_name}” books in order. Every book of the series is listed in the order of its release to ensure you read the story as it was intended.</p>
                            </div>
                        </div>
                        {/* End col-6 */}
                    </div>
            ...
            <div className="container team-section-two mt-20 d-flex flex-column align-items-center">
                <div className='d-flex align-items-center col-12 mb-3 justify-content-between' style={{ borderBottom: "3px solid #c475e3" }}>
                    <h2 className="fs-20 mb-0 tx-new fw-500 w-75" >All {seriesDetail.series_name} Books in Order with their release dates.</h2>
                ...
                <div className='mt-70'>
                    <h2 className='fw-bold fs-3 text-center tx-new text-decoration-underline'>Similar Series</h2>
                    <div className='row justify-content-around mt-20'>
                        <SeriesMap series={data ? data.similarSeries : null} link="series" noFade={true} />
                    </div>
                    <div className='d-flex justify-content-center mb-20'>
                        <Link className='fw-500 viewAllBtn' href="/series">Explore More Series</Link>
                    </div>
                </div>
            </div >
            <div className='position-fixed top-0 start-0 justify-content-center align-items-center w-100 h-100' style={{ display: seriesBooks ? "none" : "flex", background: "#ffffffeb", zIndex: "20" }}>
                <SearchBar1 />
            </div>
            <DefaultFooter />
        </>
